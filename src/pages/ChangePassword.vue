      ...
      <q-card v-bind:style="$q.screen.lt.sm ? { 'width': '80%' } : { 'width': '50%' }">
        ...
        <q-card-section>
          <q-form class="q-gutter-md" @submit="storeAuthen.changePassword(storeAuthen.email, storeAuthen.newPassword)">
            ...
            <q-input filled label="Mật khẩu mới" :type="isNewPwd ? 'password' : 'text'" v-model="storeAuthen.newPassword" :rules="[
              val => !!val || 'Mật khẩu không được rỗng',
              val => val.length > 5 || 'Mật khẩu phải tối thiểu 6 kí tự',
            ]" lazy-rules>
              <template v-slot:append>
            ...
            <q-input filled label="Nhập lại mật khẩu" :type="isreNewPwd ? 'password' : 'text'" v-model="storeAuthen.reNewPassword" :rules="[
              val => !!val || 'Mật khẩu không được rỗng',
              val => val.length > 5 || 'Mật khẩu phải tối thiểu 6 kí tự',
              val => val === storeAuthen.newPassword || 'Mật khẩu nhập lại không khớp',
            ]" lazy-rules>
              <template v-slot:append>
                <q-icon :name="isreNewPwd ? 'visibility_off' : 'visibility'" class="cursor-pointer" @click="isreNewPwd = !isreNewPwd" />
              </template>
            </q-input>
