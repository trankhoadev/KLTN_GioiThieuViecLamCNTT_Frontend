...
onMounted(async () => {
  ...
  for (let i = 0; i < storeJob.listDataSearch.length; ++i) {
    ...
    storeJob.listDataSearch[i].tennhatuyendung =
      storeJob.listDataOneRecruiter.tencongty;
    storeJob.listDataSearch[i].anhdaidien =
      storeJob.listDataOneRecruiter.anhdaidien;
  }

  /* Get amount job of every recruiter */
  for (let i = 0; i < storeJob.listRecruiter.length; ++i) {
    await storeJob.getAllPostDangTuyenDungByNhaTuyenDungId(
      storeJob.listRecruiter[i]._id
    );
  ...
  storeJob.listFavorite.filter((e) => {
    for (let i = 0; i < storeJob.listDataSearch.length; ++i) {
      if (
        e.tintuyendung._id === storeJob.listDataSearch[i]._id &&
        e.ungtuyenvien._id === storeAuthen.idUngTuyenVien
      ) {
...
watch(
  ...
  (val) => {
    const executePanigation = async () => {
      ...
      for (
        let i = (storeJob.panigateSelected - 1) * 10;
        i < storeJob.panigateSelected * 10;
        ++i
      ) {
      ...
      for (let i = 0; i < storeJob.listData.length; ++i) {
        ...
        storeJob.listData[i].tennhatuyendung =
          storeJob.listDataOneRecruiter.tencongty;
        storeJob.listData[i].anhdaidien =
          storeJob.listDataOneRecruiter.anhdaidien;
      }
    };
    executePanigation();
  ...
  <q-layout>
    <q-page-container>
      <q-page class="bg-grey-3">
        <div
          class="row justify-evenly q-pt-md container"
          style="margin: 0 auto"
        ...
        >
          <div class="col-12 q-mt-lg">
            ...
            >
              <div class="ml-auto">
                ...
                <div class="label-tag">
                  <label class="label-remote"
                    ><i class="fa-solid fa-circle-check"></i> Backend</label
                  >
                  <label class="label-remote"
                    ><i class="fa-solid fa-circle-check"></i> Frontend</label
                  >
                  <label class="label-remote"
                    ><i class="fa-solid fa-circle-check"></i> Tester</label
                  >
                  <label class="label-remote"
                    ><i class="fa-solid fa-circle-check"></i> Business
                    Analyst</label
                  >
                </div>
              </div>
              <div class="image">
                <img
                  src="https://www.topcv.vn/v4/image/it/Banner.png"
                  alt="Việc làm IT"
                  title="Việc làm IT"
                  class="w-100"
                />
          ...
          <div class="row justify-evenly bg-white q-pa-md full-width">
            ...
            <div class="col-md-2 col-12 q-my-md">
              <q-select
                color="grey-3"
                outlined
                label-color="light-green-10"
                v-model="storeJob.selectSkill"
                :options="storeJob.listSkill"
                label="Lĩnh vực"
                option-value="_id"
            ...
            <div class="col-md-2 col-12 q-my-md">
              <q-select
                ...
                outlined
                label-color="light-green-10"
                v-model="storeJob.selectLevel"
                :options="myStore.optionPosition"
                label="Cấp bậc"
              >
                <template v-slot:append>
            ...
            <div class="col-md-2 col-12 q-my-md">
              <q-select
                ...
                outlined
                label-color="light-green-10"
                v-model="storeJob.selectSalary"
                :options="myStore.optionSalary"
                label="Mức lương"
              >
                <template v-slot:append>
            ...
            <div class="col-md-2 flex flex-center">
              <q-btn
                ...
                icon="search"
                label="Tìm kiếm"
                @click="storeJob.searchAdvanced(route.params.id)"
              />
              <q-btn
                v-else
                color="light-green-10"
                icon="search"
                label="Tìm kiếm"
                @click="storeJob.searchJob()"
              />
              <q-icon
                @click="reloadData"
                class="q-ml-md cursor-pointer text-teal"
                name="cached"
                size="md"
              />
            </div>
            ...
            <div class="col-12 q-pa-md">
              <div v-if="storeJob.listDataSearch.length">
                Đã tìm thấy
                <span class="text-highlight">{{
                  storeJob.lengthResponse
                }}</span>
          ...
          <div
            v-if="storeJob.listDataSearch.length"
          ...
          >
            ...
            <div class="col-12">
              <div class="q-px-lg">
                <span class="custom-important"> Ưu tiên hiển thị: </span>

                <q-radio
                  @click="storeJob.sortPostDate(route.params.id)"
                  v-model="storeJob.sortRadio"
                  val="new"
                  label="Tin mới nhất"
                />
                <q-radio
                  @click="storeJob.sortPostSalary(route.params.id)"
                  v-model="storeJob.sortRadio"
                  val="salary"
          ...
          <div class="row bg-white full-width q-px-md justify-evenly">
            <div class="col-md-7 col-12">
              <div
                class="item highlight q-pa-none q"
                v-for="item in storeJob.listDataSearch"
                :key="item._id"
              >
                <div class="row q-my-md">
                  ...
                  <div class="col-md-7 col-12">
                    ...
                    <div class="q-pt-sm text-uppercase">
                      {{ item.tennhatuyendung }}
                    </div>
                    <div class="q-pt-sm">
                      <b>Ngày đăng:</b>
                            ...
                            <div class="row">
                              ...
                              <div class="col-10">
                                <span
                                  ...
                                  <b>{{
                                    (
                                      (new Date(item.ngayHetHan).getTime() -
                                        new Date().getTime()) /
                                      (1000 * 60 * 60 * 24)
                                    ).toFixed(0) > 0
                                      ? (
                  ...
                  <div class="col-md-3">
                    <div
                      class="row column flex-right justify-between full-height"
                    >
                      ...
                      >
                        <q-btn
                          v-if="!item.isUngTuyen"
                          class="apply-now q-mr-md"
                          color="green-7"
                          label="Ứng tuyển"
                          @click="
                            storeJob.seeDetail({
                        ...
                        />
                        <q-btn
                          v-if="!item.isYeuThich"
                          @click="
                            storeJob.yeuThichPost(
                              item._id,
                              storeAuthen.idUngTuyenVien
                            )
                          "
                          class="favorite"
                        ...
                        <q-btn
                          v-else
                          @click="storeJob.huyYeuThichPost(item.yeuThichId)"
                          class="favorite text-green"
                          icon="favorite"
                        />
                      </div>
                    </div>
              ...
              <div>
                ...
                >
                  <q-pagination
                    color="green-7"
                    v-model="storeJob.panigateSelected"
                    :max="storeJob.lengthResponse / 10 + 1"
                    :max-pages="6"
                    direction-links
                  />
                </div>
            ...
            <div class="col-md-4 col-12">
              ...
              <div
                class="list-company"
                v-for="item in storeJob.listRecruiter"
                :key="item._id"
              >
        ...
        <q-dialog v-model="storeJob.isSeeDetail">
          <q-card>
            <q-card-section class="row items-center">
              Bạn có chắc chắn muốn ứng tuyển tin "{{
                storeJob.ungTuyenSelectedData.tieude
              }}" không ?
            </q-card-section>
            ...
            <q-card-actions align="right">
              <q-btn
                ...
                v-close-popup
                @click="
                  storeJob.ungTuyenTinTuyenDung(
                    storeJob.ungTuyenSelectedData._id
                  )
                "
              />
